...
class ShapeKLine:
    ...
    def read_data(self):
        ...
        self.data = pd.read_csv(self.excel_path, usecols=['日期', '开盘', '收盘', '高', '低', '交易量', '涨跌幅'], sep=',', thousands=',')
    ...
    def draw(self):
        ...
        mpf.candlestick_ochl(ax, self.data.values, colordown=self.colordown, colorup=self.colorup, width=0.3, alpha=1)
        ax.grid(True)
        ax.xaxis_date()
        plt.show()


tool = ShapeKLine('../files/Bitcoin - 比特币历史数据_历史行情,价格,走势图表_英为财情.csv')
tool.draw()
